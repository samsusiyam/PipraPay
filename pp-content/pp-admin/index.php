...
?>
            <script>location.href = "<?php echo $site_url.$path_login; ?>";</script>
<?php
            exit();
        }
    }
            ...
            <ul class="nav w-100 flex-column gap-2">
                ...
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow w-100">
                  <?php
                      $params_perm = [
                          ...
                          ':current_brand' => $global_response_permission['response'][0]['brand_id']
                      ];
                      $response_permission = json_decode(getData($db_prefix.'permission','WHERE a_id = :a_id AND status = "active" AND brand_id != :current_brand', '* FROM', $params_perm), true);
              ...
              <li class="nav-item nav-item-transaction <?= canAccessPage(json_decode($global_response_permission['response'][0]['permission'], true), 'transaction', $global_user_response['response'][0]['role']) ? '' : 'd-none' ?>" 
                ...
                <a href="javascript:void(0)" class="nav-link d-flex align-items-center rounded">
                  ...
                  <?php
                      $params_dash = [':brand_id' => $global_response_brand['response'][0]['brand_id']];
                      $response_dashboard_info = json_decode(getData($db_prefix.'transaction',' WHERE brand_id = :brand_id AND status = "pending"', '* FROM', $params_dash), true);
                      $count = ($response_dashboard_info['status'] == true) ? count($response_dashboard_info['response']) : 0;
                  ?>
